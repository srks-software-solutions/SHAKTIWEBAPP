...
namespace SRKSDemo.Controllers
{
    public class OEEDashboardController : Controller
    {
        ...
        public ActionResult Index(int PlantID = 0, int ShopID = 0, int CellID = 0, int WorkCenterID = 0, string StartDate = null, string EndDate = null, string SummarizeAs = null, string ButtonClicked = null)
        {
            ...
            if (!dateError)
            {
                ...
                if (WorkCenterID == 0)
                {
                    if (CellID == 0)
                    {
                        if (ShopID == 0)
                        {
                            ...
                            else
                            {
                                ...
                                List<i_facilitylibrary.DAL.tblmachinedetail> machData = obj.GetmachineListforoee(PlantID);
                                //var machData = db.tblmachinedetails.Where(m => m.IsDeleted == 0 && m.PlantID == PlantID && m.IsNormalWC == 0).ToList();
                                foreach (i_facilitylibrary.DAL.tblmachinedetail row in machData)
                                {
                        ...
                        else
                        {
                            ...
                            List<i_facilitylibrary.DAL.tblmachinedetail> machData = obj.GetShopList(ShopID);
                    ...
                    else
                    {
                        ...
                        try
                        {
                            obj.deletetbloeedashboardvariablestodaysDetails2(ipAddress, CellID);
                            //dboee.tbloeedashboardvariablestodays.RemoveRange(dboee.tbloeedashboardvariablestodays.Where(x => x.IPAddress == ipAddress));
                            //dboee.SaveChanges();
                        }
                        catch (Exception)
                        ...
                        try
                        {
                            obj.deletetbloeedashboardfinalvariablesDetails2(ipAddress, CellID);
                            //dboee.tbloeedashboardfinalvariables.RemoveRange(dboee.tbloeedashboardfinalvariables.Where(x => x.IPAddress == ipAddress));
                            //dboee.SaveChanges();
                        }
                        catch (Exception)
                        ...
                        List<i_facilitylibrary.DAL.tblmachinedetail> machData = obj.GetcellListforoee(CellID);
                ...
                else
                {
                    ...
                    SummarizeAs = lowestLevel;
                    List<i_facilitylibrary.DAL.tblmachinedetail> machData = obj.GetMachineListforoee(WorkCenterID);
                    ...
                    else
                    {
                        foreach (i_facilitylibrary.DAL.tblmachinedetail row in machData)
                        {
                            ...
                            CalculateOEE(frmDate, toDate, MachineID, TimingVar, SummarizeAs);
                            CalculateSummarizedOEEWC(frmDate, toDate, MachineID, TimingVar, SummarizeAs);
                            //QualityFactor_Piweb(MachineID, frmDate.ToString("yyyy-MM-dd"));
                            //QualityFactor_PiwebReject(MachineID, frmDate.ToString("yyyy-MM-dd"));
                        }
                    }
                }
                ...
                if (lowestLevel == "WorkCentre")
                {
                    ...
                    i_facilitylibrary.DAL.tbloeedashboardfinalvariable OEEDataSummarizedToView = obj.GettbloeeDet(WorkCenterID, ipAddress, frmDate, toDate);
                ...
                if (lowestLevel == "Cell")
                {
                    ...
                    //OverAll
                    i_facilitylibrary.DAL.tbloeedashboardfinalvariable OEEDataSummarizedToView = obj.GettbloeecellDet(CellID, ipAddress, frmDate, toDate);
                    ...
                    if (OEEDataSummarizedToView != null)
                    {
                        ...
                        string firstItem = LossColors.ElementAt(0);
                        ...
                        List<KeyValuePair<string, string>> ColorLoseList = new List<KeyValuePair<string, string>>();
                        ...
                        List<i_facilitylibrary.DAL.tbloeedashboardfinalvariable> OEEDataOfWCToView = obj.GettbloeecellDet1(CellID, ipAddress, frmDate, toDate); //changed to final variables from tbloeedashboardvariables for verification purpouse
                        ...
                        int once = 0;
                        foreach (i_facilitylibrary.DAL.tbloeedashboardfinalvariable row in OEEDataOfWCToView)
                        {
                            if (once == 0)
                            {
                            }
                ...
                if (lowestLevel == "Shop")
                {
                    ...
                    i_facilitylibrary.DAL.tbloeedashboardfinalvariable OEEDataSummarizedToViewShop = obj.GettbloeeshopDet(ShopID, ipAddress, frmDate, toDate);
                    ...
                    if (OEEDataSummarizedToViewShop != null)
                    {
                        ...
                        List<i_facilitylibrary.DAL.tbloeedashboardfinalvariable> OEEDataOfCellToView = obj.GettbloeeshopDet1(ShopID, ipAddress, frmDate, toDate);
                        ...
                        {
                            foreach (i_facilitylibrary.DAL.tbloeedashboardfinalvariable row in OEEDataOfCellToView)
                            {
                                ...
                                List<i_facilitylibrary.DAL.tbloeedashboardfinalvariable> OEEDataOfWCToView = obj.GettbloeeshopDet2(CellID, ShopID, ipAddress, frmDate, toDate);
                                ...
                                foreach (i_facilitylibrary.DAL.tbloeedashboardfinalvariable WCrow in OEEDataOfWCToView)
                                {
            ...
            ViewData["WorkCenterID"] = new SelectList(obj.GetmachineList(PlantID, ShopID, WorkCenterID), "MachineID", "MachineName");
            // ViewData["WorkCenterID"] = new SelectList(db.tblmachinedetails.Where(m => m.IsDeleted == 0 && m.PlantID == PlantID && m.ShopID == ShopID && m.MachineID == WorkCenterID && m.IsNormalWC == 0), "MachineID", "MachineName");

            string plantName = obj.GettbplantDet(PlantID);
            //string plantName = db.tblplants.Where(m => m.IsDeleted == 0 && m.PlantID == PlantID).Select(m => m.PlantName).FirstOrDefault();
            string shopName = obj.GettbShopIDDet(ShopID);
            // string shopName = db.tblshops.Where(m => m.IsDeleted == 0 && m.ShopID == ShopID).Select(m => m.ShopName).FirstOrDefault();
            if (PlantID != 0)
            {
                if (CellID != 0)
                {
                    string cellName = obj.GettbcellDet(CellID);
                    ...
                    if (WorkCenterID != 0)
                    {
                        i_facilitylibrary.DAL.tblmachinedetail wcName = obj.GettbMachineDet(WorkCenterID);
                        //string wcName = db.tblmachinedetails.Where(m => m.IsDeleted == 0 && m.MachineID == WorkCenterID).Select(m => m.MachineName).FirstOrDefault();
                        ViewBag.SelectedPath = additionToPath + " => " + plantName + " => " + shopName + " => " + cellName + " => " + wcName.MachineName + " From: " + frmDate.ToString("yyyy-MM-dd") + " To: " + toDate.ToString("yyyy-MM-dd");
                        ViewData["WorkCenterID"] = new SelectList(obj.GetmachineList(PlantID, ShopID, WorkCenterID), "MachineID", "MachineName");
                        //ViewData["WorkCenterID"] = new SelectList(db.tblmachinedetails.Where(m => m.IsDeleted == 0 && m.PlantID == PlantID && m.ShopID == ShopID && m.CellID == CellID && m.IsNormalWC == 0), "MachineID", "MachineName");
                    }
                    else
                    {
                        ...
                        ViewData["WorkCenterID"] = new SelectList(obj.GetmachineList(PlantID, ShopID, WorkCenterID), "MachineID", "MachineName");
                        //ViewData["WorkCenterID"] = new SelectList(db.tblmachinedetails.Where(m => m.IsDeleted == 0 && m.PlantID == PlantID && m.ShopID == ShopID && m.IsNormalWC == 0), "MachineID", "MachineName");                        
                    }
                ...
                else
                {
                    ...
                    if (WorkCenterID != 0)
                    {
                        i_facilitylibrary.DAL.tblmachinedetail wcName = obj.GettbMachineDet(WorkCenterID);
                        //string wcName = db.tblmachinedetails.Where(m => m.IsDeleted == 0 && m.MachineID == WorkCenterID).Select(m => m.MachineName).FirstOrDefault();
                        ViewBag.SelectedPath = additionToPath + " => " + plantName + " => " + shopName + " => " + wcName.MachineName + " From: " + frmDate.ToString("yyyy-MM-dd") + " To: " + toDate.ToString("yyyy-MM-dd");
                        ViewData["WorkCenterID"] = new SelectList(obj.GetmachineList(PlantID, ShopID, WorkCenterID), "MachineID", "MachineName");
                        //ViewData["WorkCenterID"] = new SelectList(db.tblmachinedetails.Where(m => m.IsDeleted == 0 && m.PlantID == PlantID && m.ShopID == ShopID && m.IsNormalWC == 0), "MachineID", "MachineName");
                    }
                    else
        ...
        public string GetMacInvNo(int machineId)
        {
            ...
            obj = new Dao1(_conn);
            string macinv = null;
            macinv = Convert.ToString(obj.GettbMachineDet2(machineId)); //changed for verification purpouse on 2020-07-06
            // macinv = db.tblmachinedetails.Where(m => m.IsDeleted == 0 && m.MachineID == machineId && m.IsNormalWC == 0).Select(m => m.MachineName).FirstOrDefault();
            return macinv;
        }
        ...
        public void CalculateSummarizedOEEPlant(DateTime fromdate, DateTime todate, int FactorID, string TimeType, string SummarizeAs = null)
        {
            ...
            if (TimeType == "Today")
            {
                ...
                string ipAddress = GetIPAddressOf();
                List<i_facilitylibrary.DAL.tbloeedashboardvariablestoday> getPlant = obj.GettbloeeListDet1(FactorID, ipAddress, fromdate, todate);
                ...
                foreach (i_facilitylibrary.DAL.tbloeedashboardvariablestoday PlantRow in getPlant)
                {
                    ...
                    List<i_facilitylibrary.DAL.tbloeedashboardvariablestoday> OEEDataToSummarize = obj.GettbloeeListDet2(id, ipAddress, fromdate, todate);
            ...
            else if (TimeType == "GodHours" || TimeType == "NoBlue")
            {
                ...
                //Gather Hierarchy Details
                string PlantIDS = null, ShopIDS = null, CellIDS = null, WCID = null;
                List<i_facilitylibrary.DAL.tbloeedashboardvariable> getPlant = obj.GettbloeeListDet3(FactorID, fromdate, todate);
                ...
                foreach (i_facilitylibrary.DAL.tbloeedashboardvariable PlantRow in getPlant)
                {
                    double green = 0, blue = 0, OperatingTime = 0, DownTimeBreakdown = 0, ROALossess = 0, AvailableTime = 0, SettingTime = 0;
                    double SummationOfSCTvsPP = 0, MinorLosses = 0;
                    double ScrapQtyTime = 0, ReWOTime = 0;
                    int id = Convert.ToInt32(PlantRow);
                    List<i_facilitylibrary.DAL.tbloeedashboardvariable> OEEDataToSummarize = obj.GettbloeeListDet4(id, fromdate, todate);
                    ...
                    CellIDS = FactorID.ToString();
                    int once = 0;
                    foreach (i_facilitylibrary.DAL.tbloeedashboardvariable row in OEEDataToSummarize)
                    {
                        if (once == 0)
                        {
                            PlantIDS = Convert.ToString(row.PlantID);
                        ...
                        //Quality
                        ScrapQtyTime = QualityFactor_Piweb(Convert.ToInt32(PlantIDS), UsedDateForExcel.ToString("yyyy-MM-dd"), 1);
                        ScrapQtyTime += Convert.ToDouble(row.ScrapQtyTime);
                        ReWOTime += Convert.ToDouble(row.ReWOTime);
                    }
        ...
        public void CalculateSummarizedOEEShop(DateTime fromdate, DateTime todate, int FactorID, string TimeType, string SummarizeAs = null)
        {
            ...
            if (TimeType == "Today")
            {
                ...
                List<i_facilitylibrary.DAL.tbloeedashboardvariablestoday> getShop = obj.GettbloeeShopListDet1(FactorID, ipAddress, fromdate, todate);
                ...
                foreach (i_facilitylibrary.DAL.tbloeedashboardvariablestoday ShopRow in getShop)
                {
                    double green = 0, blue = 0, OperatingTime = 0, DownTimeBreakdown = 0, ROALossess = 0, AvailableTime = 0, SettingTime = 0;
                    double SummationOfSCTvsPP = 0, MinorLosses = 0;
                    double ScrapQtyTime = 0, ReWOTime = 0;
                    int id = Convert.ToInt32(ShopRow.ShopID);
                    List<i_facilitylibrary.DAL.tbloeedashboardvariablestoday> OEEDataToSummarize = obj.GettbloeeShopList1Det1(id, ipAddress, fromdate, todate);
            ...
            else if (TimeType == "GodHours" || TimeType == "NoBlue")
            {
                ...
                //Gather Hierarchy Details
                string PlantIDS = null, ShopIDS = null, CellIDS = null, WCID = null;
                List<i_facilitylibrary.DAL.tbloeedashboardvariable> getShop = obj.GettbloeeShopListDet2(FactorID, fromdate, todate);
                ...
                foreach (i_facilitylibrary.DAL.tbloeedashboardvariable Row in getShop)
                {
                    ...
                    List<i_facilitylibrary.DAL.tbloeedashboardvariable> OEEDataToSummarize = obj.GettbloeeShopList1Det2(id, fromdate, todate);
                    ...
                    foreach (i_facilitylibrary.DAL.tbloeedashboardvariable row in OEEDataToSummarize)
                    {
                        ...
                        //Quality
                        ScrapQtyTime = QualityFactor_Piweb(Convert.ToInt32(ShopIDS), UsedDateForExcel.ToString("yyyy-mm-dd"), 5);
                        ScrapQtyTime += Convert.ToDouble(row.ScrapQtyTime);
                        ReWOTime += Convert.ToDouble(row.ReWOTime);
                    }
                    int MachineCount = 0;
                    ...
                    try
                    {
                        ...
                        SqlCommand cmdFinalInsertRows = new SqlCommand("INSERT INTO " + databaseName + ".tbloeedashboardfinalvariables(PlantID,ShopID,CellID,WCID,StartDate,EndDate,OEE,Availability,Performance,Quality,IsOverallShopWise,IsOverallCellWise,IsOverallWCWise,Loss1Name,Loss1Value,Loss2Name,Loss2Value,Loss3Name,Loss3Value,Loss4Name,Loss4Value,Loss5Name,Loss5Value,IPAddress,CreatedOn,CreatedBy,IsDeleted)VALUES('" + Convert.ToInt32(PlantIDS) + "','" + Convert.ToInt32(ShopRow) + "','" + Convert.ToInt32(CellIDS) + "','" + Convert.ToInt32(WCID) + "','" + fromdate.ToString("yyyy-MM-dd HH:mm:ss") + "','" + todate.ToString("yyyy-MM-dd HH:mm:ss") + "','" + OEEFactor + "','" + AvaillabilityFactor + "','" + EfficiencyFactor + "','" + QualityFactor + "','" + isShopWise + "','" + isCellWise + "','" + isWCWise + "','" + lossCode1 + "','" + lossCodeVal1 + "','" + lossCode2 + "','" + lossCodeVal2 + "','" + lossCode3 + "','" + lossCodeVal3 + "','" + lossCode4 + "','" + lossCodeVal4 + "','" + lossCode5 + "','" + lossCodeVal5 + "','" + IPAddress + "','" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','" + 1 + "','" + 0 + "');", mcFinalInsertRows.msqlConnection);
                        cmdFinalInsertRows.ExecuteNonQuery();
                    }
                    catch (Exception)
                    {
        ...
        public void CalculateSummarizedOEECell(DateTime fromdate, DateTime todate, int FactorID, string TimeType, string SummarizeAs = null)
        {
            ...
            double TotalDay = todate.Subtract(fromdate).TotalDays;
            string ipAddress = GetIPAddressOf();
            //string machineid = GetMacInvNo(machineId);
            ...
            if (TimeType == "Today")
            {
                ...
                //Gather Hierarchy Details
                string PlantIDS = null, ShopIDS = null, CellIDS = null, WCID = null;
                List<i_facilitylibrary.DAL.tbloeedashboardvariablestoday> getCell = obj.GettbloeeListDetforcell1(FactorID, ipAddress, fromdate, todate);
                ...
                foreach (i_facilitylibrary.DAL.tbloeedashboardvariablestoday CellRow in getCell)
                {
                    double green = 0, blue = 0, OperatingTime = 0, DownTimeBreakdown = 0, ROALossess = 0, AvailableTime = 0, SettingTime = 0;
                    double SummationOfSCTvsPP = 0, MinorLosses = 0;
                    double ScrapQtyTime = 0, ReWOTime = 0;
                    int id = Convert.ToInt32(CellRow.CellID);
                    List<i_facilitylibrary.DAL.tbloeedashboardvariablestoday> OEEDataToSummarize = obj.GettbloeeListDetforcell(id, ipAddress, fromdate, todate);
            ...
            else if (TimeType == "GodHours" || TimeType == "NoBlue")
            {
                ...
                List<i_facilitylibrary.DAL.tbloeedashboardvariable> getCell = obj.GettbloeecelllistDet1(FactorID, fromdate, todate);
                ...
                foreach (i_facilitylibrary.DAL.tbloeedashboardvariable Row in getCell)
                {
                    ...
                    List<i_facilitylibrary.DAL.tbloeedashboardvariable> OEEDataToSummarize = obj.GettbloeecelllistDet2(id, fromdate, todate);
                    ...
                    foreach (i_facilitylibrary.DAL.tbloeedashboardvariable row in OEEDataToSummarize)
                    {
                        ...
                        //Quality
                        ScrapQtyTime = QualityFactor_Piweb(Convert.ToInt32(CellIDS), UsedDateForExcel.ToString("yyyy-MM-dd"), 3);
                        ScrapQtyTime += Convert.ToDouble(row.ScrapQtyTime);
                        ReWOTime += Convert.ToDouble(row.ReWOTime);
                    ...
                    //Machine Wise Data
                    int MachineCount = 0;
                    MachineCount = obj.GettbloeeListDetailsmacdata1(id, ipAddress, fromdate, todate).Count(); //added on 2020-07-07
                    //MachineCount = db.tbloeedashboardfinalvariables.Where(m => m.CellID == CellRow && m.StartDate >= fromdate && m.StartDate <= todate && m.IPAddress == ipAddress && m.IsOverallWCWise == 1).Count();
                    ...
                    using (MsqlConnection mcFinalInsertRows = new MsqlConnection())
                    {
                        ...
                        try
                        {
                                int Machineid = Convert.ToInt32(WCID);
                                DateTime correcteddate = Convert.ToDateTime(fromdate.ToString("yyyy-MM-dd 00:00:00"));
                                db.tbloeedashboardfinalvariables.RemoveRange(db.tbloeedashboardfinalvariables.Where(x => x.WCID == Machineid && x.IsOverallCellWise == isCellWise && x.StartDate == correcteddate));
                                db.SaveChanges();

                                SqlCommand cmdFinalInsertRows = new SqlCommand("INSERT INTO " + databaseName + ".tbloeedashboardfinalvariables(PlantID,ShopID,CellID,WCID,StartDate,EndDate,OEE,Availability,Performance,Quality,IsOverallShopWise,IsOverallCellWise,IsOverallWCWise,Loss1Name,Loss1Value,Loss2Name,Loss2Value,Loss3Name,Loss3Value,Loss4Name,Loss4Value,Loss5Name,Loss5Value,IPAddress,CreatedOn,CreatedBy,IsDeleted)" +
                                "VALUES('" + Convert.ToInt32(PlantIDS) + "','" + Convert.ToInt32(ShopIDS) + "','" + Convert.ToInt32(CellRow) + "','" + Convert.ToInt32(WCID) + "','" + fromdate.ToString("yyyy-MM-dd HH:mm:ss") + "','" + todate.ToString("yyyy-MM-dd HH:mm:ss") + "','" + OEEFactor + "','" + AvaillabilityFactor + "','" + EfficiencyFactor + "','" + QualityFactor + "','" + isShopWise + "','" + isCellWise + "','" + isWCWise + "','" + lossCode1 + "','" + lossCodeVal1 + "','" + lossCode2 + "','" + lossCodeVal2 + "','" + lossCode3 + "','" + lossCodeVal3 + "','" + lossCode4 + "','" + lossCodeVal4 + "','" + lossCode5 + "','" + lossCodeVal5 + "','" + IPAddress + "','" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','" + 1 + "','" + 0 + "');", mcFinalInsertRows.msqlConnection);
                            cmdFinalInsertRows.ExecuteNonQuery();
                        }
                        catch (Exception ex)
                        {
        ...
        private string GetMacInvNo(object machineId)
        {
        ...
        public void CalculateSummarizedOEEWC(DateTime fromdate, DateTime todate, int FactorID, string TimeType, string SummarizeAs = null)
        {
            ...
            else if (TimeType == "GodHours" || TimeType == "NoBlue")
            {
                ...
                //Gather Hierarchy Details
                string PlantIDS = null, ShopIDS = null, CellIDS = null, WCID = null;
                List<i_facilitylibrary.DAL.tbloeedashboardvariable> getWC = obj.GettbloeecellvariableListDet1(FactorID, fromdate, todate);
                ...
                foreach (i_facilitylibrary.DAL.tbloeedashboardvariable WCRow in getWC)
                {
                    double green = 0, blue = 0, OperatingTime = 0, DownTimeBreakdown = 0, ROALossess = 0, AvailableTime = 0, SettingTime = 0;
                    double SummationOfSCTvsPP = 0, MinorLosses = 0;
                    double ScrapQtyTime = 0, ReWOTime = 0;
                    ...
                    List<i_facilitylibrary.DAL.tbloeedashboardvariable> OEEDataToSummarize = obj.GettbloeeWCvariableListDet1(id, fromdate, todate);
                    ...
                    foreach (i_facilitylibrary.DAL.tbloeedashboardvariable row in OEEDataToSummarize)
                    {
                        ...
                        //Quality
                        ScrapQtyTime = QualityFactor_Piweb(Convert.ToInt32(WCID), UsedDateForExcel.ToString("yyyy-MM-dd"), 4);
                        ScrapQtyTime += Convert.ToDouble(row.ScrapQtyTime);
                        ReWOTime += Convert.ToDouble(row.ReWOTime);
                    }
        ...
        public void CalculateOEE(DateTime fromdate, DateTime todate, int MachineID, string TimeType, string SummarizeAs = null)
        {
            ...
            {
                List<i_facilitylibrary.DAL.tbloeedashboardvariable> OEEDataPresent = obj.GettbloeeWCvariableListDet2(MachineID, UsedDateForExcel);
        ...
        public JsonResult GetShop(int PlantID)
        {
            ...
            obj1 = new Dao(_conn);
            obj = new Dao1(_conn);
            SelectList ShopData = new SelectList(obj.GetShop1List(PlantID), "ShopID", "ShopName");
        ...
        public JsonResult GetCell(int ShopID)
        {
            ...
            obj1 = new Dao(_conn);
            obj = new Dao1(_conn);
            SelectList CellData = new SelectList(obj.GetCell1List(ShopID), "CellID", "CellName");
        ...
        public JsonResult GetWC_Cell(int CellID)
        {
            ...
            obj1 = new Dao(_conn);
            obj = new Dao1(_conn);
            SelectList MachineData = new SelectList(obj.GetmachinecellList(CellID), "MachineID", "MachineName");
            //var MachineData = new SelectList(db.tblmachinedetails.Where(m => m.IsDeleted == 0 && m.CellID == CellID && m.IsNormalWC == 0), "MachineID", "MachineName");
        ...
        public JsonResult GetWC_Shop(int ShopID)
        {
            ...
            obj1 = new Dao(_conn);
            obj = new Dao1(_conn);
            SelectList MachineData = new SelectList(obj.GetmachineshopcellList(ShopID), "MachineID", "MachineName");
            //var MachineData = new SelectList(db.tblmachinedetails.Where(m => m.IsDeleted == 0 && m.ShopID == ShopID && m.CellID.Equals(null) && m.IsNormalWC == 0), "MachineID", "MachineName");
            return Json(MachineData, JsonRequestBehavior.AllowGet);
        }
        ...
        public int GetMinorLosses(string CorrectedDate, int MachineID, string Colour)
        {
            ...
            int minorloss = 0;
            List<i_facilitylibrary.DAL.tblmode> modeData = obj.GetmodeList(MachineID, CorrectedDate);
        ...
        public int GetOPIDleBreakDown(string CorrectedDate, int MachineID, string Colour)
        {
            ...
            using (MsqlConnection mc = new MsqlConnection())
            {
                ...
                //operating
                mc.open();
                String query1 = "SELECT count(ID) From " + databaseName + ".tbldailyprodstatus WHERE CorrectedDate='" + CorrectedDate + "' AND MachineID=" + MachineID + " AND ColorCode='" + Colour + "'";
        ...
        public double GetSettingTime(string UsedDateForExcel, int MachineID)
        {
            ...
            List<i_facilitylibrary.DAL.tbllossofentry> SettingData = obj.GettbllossofentryDet1(SettingIDs, MachineID, UsedDateForExcel);
            ...
            foreach (i_facilitylibrary.DAL.tbllossofentry row in SettingData)
            {
        ...
        public double GetDownTimeBreakdown(string UsedDateForExcel, int MachineID)
        {
            ...
            List<i_facilitylibrary.DAL.tblbreakdown> BreakdownData = obj.GettblbreakdownDet1(MachineID, UsedDateForExcel);
            ...
            foreach (i_facilitylibrary.DAL.tblbreakdown row in BreakdownData)
            {
        ...
        public double GetSummationOfSCTvsPP(string UsedDateForExcel, int MachineID)
        {
            ...
            List<i_facilitylibrary.DAL.tblhmiscreen> PartsDataAll = obj.GettblhmiscreensDet1(MachineID, UsedDateForExcel);
            ...
            foreach (i_facilitylibrary.DAL.tblhmiscreen row in PartsDataAll)
            {
                string partNo = row.PartNo;
                ...
                if (!OccuredWOs.Contains(occuredwo))
                {
                    ...
                    List<i_facilitylibrary.DAL.tblhmiscreen> PartsData = obj.GettblhmiscreensDet2(MachineID, UsedDateForExcel, woNo, partNo, opNo);
                    ...
                    List<i_facilitylibrary.DAL.tbl_multiwoselection> mulitwoData = obj.Gettbl_multiwoselectionDet1(woNo, partNo, opNo);
                    ...
                    int stdCuttingTime = 0;
                    i_facilitylibrary.DAL.tblmasterparts_st_sw stdcuttingTimeData = obj.Gettblmasterparts_st_swDet3(opNo, partNo);
        ...
        public double GetGreen(string UsedDateForExcel, DateTime TSstartTime, DateTime TSendTime, int MachineID)
        {
            ...
            try
            {
                ...
                {
                    mc.open();
                    String query = "SELECT * From " + databaseName + ".tblmode WHERE MachineID = '" + MachineID + "' and ColorCode = 'green' and IsCompleted = 1  and "
        ...
        //public double Plannedbreakgreen(string UsedDateForExcel, DateTime TSstartTime, DateTime TSendTime, DateTime plStarttime, DateTime plEndtime, int MachineID)
        //{
        //    _conn = new ConnectionFactory();
        //    obj1 = new Dao(_conn);
        //    obj = new Dao1(_conn);
        //    double PlbrOperatingTime = 0;
        //    double FirstRowDur = 0;
        //    double LastRowDur = 0;
        //   // DateTime StartTime =  Convert.ToDateTime(System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
        ...
        //        {
        //            mc.open();
        //            string query1 = "SELECT * From " + databaseName + ".tblplannedbreak WHERE MachineID = '" + MachineID + "' and StartTime = '" + plStarttime + "' and EndTime = '" + plEndtime + "' and IsDeleted = 0 ";

        //            String query = "SELECT * From " + databaseName + ".tbllivemode WHERE MachineID = '" + MachineID + "' and ColorCode = 'green' and IsCompleted = 1  and (StartTime >= '" + plStarttime + "' and EndTime <= '" + plEndtime + "') ";
        //            SqlDataAdapter da1 = new SqlDataAdapter(query, mc.msqlConnection);
        //            da1.Fill(GreenData);
        //            mc.close();
        //        }
        ...
        //                    IndividualDur -= EndDurationExtra;
        //                }
        //                PlbrOperatingTime += IndividualDur;
        //            }
        //        }
        //    }
        //    catch (Exception e)
        //    { }
        //    return Math.Round(PlbrOperatingTime / 60, 2);
        ...
        public string LossHierarchy(int LossCode)
        {
            ...
            i_facilitylibrary.DAL.tbllossescode lossdata = obj1.GetLossDet(LossCode);
            ...
            else if (level == 2)
            {
                ...
                i_facilitylibrary.DAL.tbllossescode lossdata1 = obj1.GetLossDet(lossLevel1ID);
            ...
            else if (level == 3)
            {
                ...
                i_facilitylibrary.DAL.tbllossescode lossdata1 = obj1.GetLossDet(lossLevel1ID);
                //var lossdata1 = dbloss.tbllossescodes.Where(m => m.LossCodeID == lossLevel1ID).FirstOrDefault();
                i_facilitylibrary.DAL.tbllossescode lossdata2 = obj1.GetLossDet(lossLevel2ID);
        ...
        public void CalculateOEEToday(DateTime fromdate, DateTime todate, int MachineID, string TimeType, string SummarizeAs = null)
        {
            ...
            string ipAddress = GetIPAddressOf();
            List<i_facilitylibrary.DAL.tbloeedashboardvariablestoday> OEEDataPresent = obj.GettbloeeListDet3(MachineID, ipAddress, UsedDateForExcel);
            ...
            if (OEEDataPresent.Count == 0)
            {
                ...
                //Quality
                ScrapQtyTime = QualityFactor_Piweb(MachineID, UsedDateForExcel.ToString("yyyy-MM-dd"), 4);
                if (ScrapQtyTime < 0)
                {
                    ScrapQtyTime = 0;
                }
                ...
                //    AvailableTime = (DateTime.Now - Convert.ToDateTime(DateTime.Now.AddDays(-1).ToString("yyyy-MM-dd 06:00:00"))).TotalMinutes;
                //}
                OperatingTime = (green + MinorLosses);
        ...
        public double QualityFactor_Piweb(int MachineID, string UsedDateForExcel, int scpid)
        {
        ...
        public JsonResult GetDonutData()
        {
            ...
            DataTable GetDistinctLossDT = new DataTable();
            string GetDistinctLossQuery = "SELECT Blue FROM " + databaseName + ".tbloeedashboardvariables where WCID = 1";
            using (MsqlConnection mc = new MsqlConnection())
            {
                mc.open();
                SqlDataAdapter GetDistinctLossDA = new SqlDataAdapter(GetDistinctLossQuery, mc.msqlConnection);
        ...
        public string FetchPlant()
        {
            ...
            List<int?> sensormachine = db.tblmachinedetails.Where(m => m.IsDeleted == 0).Select(m => m.PlantID).Distinct().ToList();
            foreach (int? sensormachinedet in sensormachine)
            {
        ...
        public string FetchShop(int PlantID)
        {
            string res = "";
            List<PSCDetails> listPSCDetails = new List<PSCDetails>();
            List<int?> sensormachine = db.tblmachinedetails.Where(m => m.IsDeleted == 0 && m.PlantID == PlantID).Select(m => m.ShopID).Distinct().ToList();
            foreach (int? sensormachinedet in sensormachine)
            {
                PSCDetails objPSCDetails = new PSCDetails();
        ...
        public string Fetch_Shop(int ShopID)
        {
            string res = "";
            List<PSCDetails> listPSCDetails = new List<PSCDetails>();
            List<int?> sensormachine = db.tblmachinedetails.Where(m => m.IsDeleted == 0 && m.ShopID == ShopID).Select(m => m.ShopID).Distinct().ToList();
            foreach (int? sensormachinedet in sensormachine)
            {
                PSCDetails objPSCDetails = new PSCDetails();
        ...
        public string Fetchcell(int ShopID)
        {
            string res = "";
            List<PSCDetails> listPSCDetails = new List<PSCDetails>();
            List<int?> sensormachine = db.tblmachinedetails.Where(m => m.IsDeleted == 0 && m.ShopID == ShopID).Select(m => m.CellID).Distinct().ToList();
            foreach (int? sensormachinedet in sensormachine)
            {
                PSCDetails objPSCDetails = new PSCDetails();
        ...
        {
            string res = "";
            var celldetails = (from s in db.tblmachinedetails where s.CellID == CellID && s.IsDeleted == 0 select new { Value = s.MachineID, Text = s.MachineName }).ToList();
